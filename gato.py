...
class Gato(Animal):
    ...
    def resumen(self):
        ...
        if self.estado == True: 
            return "[%s/%s/%s"%(fecha.day,fecha.month,fecha.year) + " %s:%s"%(fecha.hour,fecha.minute)+"] "+ self.nombre + ", Energia: " + self.energia + ", X: " + self.posicion_x + ", Y: " + self.posicion_y + ", Gato , vivo." 
        else:
            return "[%s/%s/%s"%(fecha.day,fecha.month,fecha.year) + " %s:%s"%(fecha.hour,fecha.minute)+"] "+ self.nombre + ", Energia: " + self.energia + ", X: " + self.posicion_x + ", Y: " + self.posicion_y + ", Gato , muerto." 


    def comer(self,gramos):
        self.energia = self.energia + (12 + gramos)
    
    def posibleEnergia(self,gramos):
        return self.energia + (12 + gramos)

    def validarComerRaton(self,x,y,gramos):
        energia_consumida = int(self.calcularDistancia(x,y)/2)
        fecha = datetime.datetime.now()
        if self.estado == True:
            if self.posibleEnergia(gramos) >= energia_consumida:
